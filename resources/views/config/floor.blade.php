                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach ($aResponse as $value)
                            <form 
                                action="{{ route('config/floor.update', ['floor' => $value['id']]) }}" 
                                method="POST" 
                                name="form-floor-{{ $value['id'] }}"
                            >
                                @csrf
                                @method('PATCH')
                                <tr>
                                    ...
                                    <td>
                                        <div class="row">
                                            <div class="col-6">
                                                    <button style="border: none; background-color:#fff" type="button" onclick="deleteById('config/floor/{{ $value['id'] }}', {{ $value['id'] }}, '{{ $value['name'] }}')">
